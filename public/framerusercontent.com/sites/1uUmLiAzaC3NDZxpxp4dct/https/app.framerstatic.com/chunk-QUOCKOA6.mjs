...
function PopChild({ children, isPresent: isPresent2, anchorX, root }) {
  ...
  return jsx(PopChildMeasure, { isPresent: isPresent2, childRef: ref, sizeRef: size, children: React2.cloneElement(children, { ref: composedRef }) });
}
...
var PresenceChild = ({ children, initial, isPresent: isPresent2, onExitComplete, custom, presenceAffectsLayout, mode, anchorX, root }) => {
  ...
  if (mode === "popLayout") {
    children = jsx2(PopChild, { isPresent: isPresent2, anchorX, root, children });
  }
...
var AnimatePresence = ({ children, custom, initial = true, onExitComplete, presenceAffectsLayout = true, mode = "sync", propagate = false, anchorX = "left", root }) => {
  ...
  return jsx3(Fragment, { children: renderedChildren.map((child) => {
    ...
    return jsx3(PresenceChild, { isPresent: isPresent2, initial: !isInitialRender.current || initial ? void 0 : false, custom, presenceAffectsLayout, mode, root, onExitComplete: isPresent2 ? void 0 : onExit, anchorX, children: child }, key);
  }) });
};
...
import { useContext as useContext8, useRef as useRef7, useInsertionEffect as useInsertionEffect2, useEffect as useEffect5 } from "react";
function useVisualElement(Component3, visualState, props, createVisualElement, ProjectionNodeConstructor) {
  ...
  const wantsHandoff = useRef7(Boolean(optimisedAppearId) && !window.MotionHandoffIsComplete?.(optimisedAppearId) && window.MotionHasOptimisedAnimation?.(optimisedAppearId));
  useIsomorphicLayoutEffect(() => {
    if (!visualElement)
      return;
    isMounted.current = true;
...
import { useRef as useRef8, useCallback as useCallback5, useEffect as useEffect7 } from "react";
...
function useScroll({ container, target, ...options } = {}) {
  ...
  const scrollAnimation = useRef8(null);
  const needsStart = useRef8(false);
...
import { useRef as useRef9, useContext as useContext13, useEffect as useEffect9 } from "react";
...
import { useRef as useRef10, useState as useState6, useCallback as useCallback7 } from "react";
...
import { forwardRef as forwardRef2, useRef as useRef12, useEffect as useEffect15 } from "react";
...
//# sourceMappingURL=chunk-QUOCKOA6.mjs.map
